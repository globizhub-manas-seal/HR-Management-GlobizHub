...
export class ShiftSwapController {
  ...
  async createRequest(@Request() req, @Body() dto: CreateSwapRequestDto) {
    return this.shiftSwapService.createRequest(
      req.user.sub,
      req.user.companyId,
      dto,
    );
  ...
  async approveRequest(
    @Request() req,
    @Param('id') id: string,
    @Body() dto: ApproveSwapRequestDto,
  ) {
  ...
  async cancelRequest(
    ...
    return this.shiftSwapService.cancelRequest(
      req.user.sub,
      req.user.role,
      id,
      dto.reason,
  ...
  async approveCancellation(
    ...
    return this.shiftSwapService.approveCancellation(
      req.user.sub,
      req.user.role,
      id,
      dto.approve,
  ...
  async getMyRequests(@Request() req) {
    return this.shiftSwapService.getMyRequests(
      req.user.sub,
      req.user.companyId,
      req.user.role,
    );
